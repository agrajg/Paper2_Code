  ...
  cat('# --------------------------------------------------', '\n')
  {
    ...
    for(i in seq(from = 1, to = nfeat(bed_amenities.tokens.dfm_trim.tfidf), by = 200)){
      ...
      cat('# -------------------------------', '\n')
      {
        ...
        cat('# Writing the csv ... ', '\n')
        fwrite(x = text.DT, 
               file = paste(project.path, "Output/TEMP/ForH2O_Rental_GLRM/", "22_01_bed_am_DT_", as.character(i), ".csv", sep = ""), 
               append = FALSE)
        rm(text.DT)
        gc()
        cat('# Final rental level ameneties data and save.', '\n')
  ...
  cat('# --------------------------------------------------', '\n')
  {
    ...
    for(i in seq(from = 1, to = nfeat(desc.tokens.dfm_trim.tfidf), by = 200)){
      ...
      cat('# -------------------------------', '\n')
      {
        ...
        cat('# Writing the csv ... ', '\n')
        fwrite(x = text.DT, 
               file = paste(project.path, "Output/TEMP/ForH2O_Rental_GLRM/", "22_01_desc_DT_", as.character(i), ".csv", sep = ""), 
               append = FALSE)
        rm(text.DT)
        gc()
        cat('# Final rental level desc data and save.', '\n')
...
cat('# -------------------------------', '\n')
{
  ...
  fwrite(x = rental.data.allchar, 
         file = paste(project.path, "Output/TEMP/ForH2O_Rental_GLRM/", "22_01_all_char_DT", ".csv", sep = ""), 
         append = FALSE)
...
#          file = paste(project.path, "Output/TEMP/ForH2O_Rental_GLRM/", "22_01_PID_DT", ".csv", sep = ""), 
#          append = FALSE)
#   rm(pid.data.temp)
#   gc()
#   cat('# Final rental level other char data and save.', '\n')
