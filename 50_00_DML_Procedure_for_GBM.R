cat('* Procedure for GBM, Iteration = ', i , '\n')
{
  ...
  cat('* Estimate nuisance function with GBM, Iteration = ', i , '\n')
  {
    for(y in Y){
      # y=Z[1]
      cat('* y variable is : ' , y , '\n')
      ...
      cat('* Run GBM for y : ', y, ', predict and combine residuals','\n')
      {
        cat('* Run GBM , Iteration = ', i , '\n')
        {
        ...
        cat('* Storing GBM details ... ', '\n')
        {
          ...
          Results_ML_Performance$method_name <- c(Results_ML_Performance$method_name, "GBM")
          Results_ML_Performance$train_MSE <- c(Results_ML_Performance$train_MSE, h2o.performance(GBM_fit, train = TRUE, xval = FALSE)@metrics$MSE)
          Results_ML_Performance$xval_MSE <- c(Results_ML_Performance$xval_MSE, h2o.performance(GBM_fit, train = FALSE, xval = TRUE)@metrics$MSE)
          Results_ML_Performance$parameters <- c(Results_ML_Performance$parameters, paste("learn_rate = ", GBM_fit@allparameters$learn_rate, ",",
                                                                            "ntrees = ", GBM_fit@allparameters$ntrees, ",",
                                                                            "max_depth = ", GBM_fit@allparameters$max_depth, ",",
                                                                            "min_rows = ", GBM_fit@allparameters$min_rows, ",",
                                                                            "nbins = ", GBM_fit@allparameters$nbins, 
                                                                            sep = " "))
          Results_ML_Performance$seed <- c(Results_ML_Performance$seed, GBM_fit@allparameters$seed)
        }
        
        cat('* Create Vector of residuls of y = ', y ,'\n')
        {
          if(h2o.isfactor(final_df_h2o[,y])){
            resyX.pool <- (h2o.asnumeric(final_df_h2o[,y]) - h2o.getFrame(GBM_fit@model[["cross_validation_holdout_predictions_frame_id"]][["name"]])$p1) %>% as.matrix()
            cat('*', y, 'is a BINARY variable.' , '\n')
          } else {
            resyX.pool <- (h2o.asnumeric(final_df_h2o[,y]) - h2o.getFrame(GBM_fit@model[["cross_validation_holdout_predictions_frame_id"]][["name"]])) %>% as.matrix()
            cat('*', y, 'is a CONTINIOUS variable.' , '\n')
          }
        ...
        cat('* Remove objects created by GBM on y : ', y ,'\n')
        {
    ...
    for(d in D){
      ...
      cat('* Run GBM for d : ', d, ', predict and combine residuals','\n')
      {
        cat('* Run GBM , Iteration = ', i , '\n')
        {
          seed = sample(1:999999, 1, replace=TRUE)
        ...
        {
          if(h2o.isfactor(final_df_h2o[,d])){
            resdX.pool <- (h2o.asnumeric(final_df_h2o[,d]) - h2o.getFrame(GBM_fit@model[["cross_validation_holdout_predictions_frame_id"]][["name"]])$p1) %>% as.matrix()
            cat('*', d, 'is a BINARY variable.' , '\n')
          } else {
            resdX.pool <- (h2o.asnumeric(final_df_h2o[,d]) - h2o.getFrame(GBM_fit@model[["cross_validation_holdout_predictions_frame_id"]][["name"]])) %>% as.matrix()
            cat('*', d, 'is a CONTINIOUS variable.' , '\n')
          }
        ...
        cat('* Remove objects created by GBM on d : ', d ,'\n')
        {
    ...
    for(z in Z){
      ...
      cat('* Run GBM for z : ', z, ', predict and combine residuals','\n')
      {
        cat('* Run GBM , Iteration = ', i , '\n')
        {
          seed = sample(1:999999, 1, replace=TRUE)
        ...
        {
          if(h2o.isfactor(final_df_h2o[,z])){
            reszX.pool <- (h2o.asnumeric(final_df_h2o[,z]) - h2o.getFrame(GBM_fit@model[["cross_validation_holdout_predictions_frame_id"]][["name"]])$p1) %>% as.matrix()
            cat('*', z, 'is a BINARY variable.' , '\n')
          } else {
            reszX.pool <- (h2o.asnumeric(final_df_h2o[,z]) - h2o.getFrame(GBM_fit@model[["cross_validation_holdout_predictions_frame_id"]][["name"]])) %>% as.matrix()
            cat('*', z, 'is a CONTINIOUS variable.' , '\n')
          }
        ...
        cat('* Remove objects created by GBM on z : ', z ,'\n')
        {
  ...
  cat('* Perform GMM estimation for DML1, Iteration = ', i , '\n')
  {
    ...
    cat('* Storing Results of DML 1 ... ', '\n')
    {
      ...
      Results_DML$method_name <- c(Results_DML$method_name, "GBM")
