...
cat('* Take all inputs ...', '\n')
{
  filename.begin = "63_01"                                                              # For I/O Setup
  ...
  # GLM Parameters
  hparams_GLM = list(alpha = c(1))                       # *** Change this in the final program
...
cat('* I/O Setup ...', '\n')
{
  ...
  cat('* FILE : 63_00_Set_project_paths_and_create_folders.R', '\n')
  source(file = '63_00_Set_project_paths_and_create_folders.R')
}

cat('* Importing and preparing the data for DML.', '\n')
{
  cat('- Maximum text features to use = ', max_text_features, '\n')
  cat('* FILE : 63_00_Setting_up_data.R', '\n')
  source(file = "63_00_Setting_up_data.R")
  
  cat('* FILE : 63_00_Defining_variables.R', '\n')
  source(file = '63_00_Defining_variables.R')                                             # Change this program in the final program
}

cat('* FILE : 63_00_DML_Print_All_ML_Parameters.R', '\n')
source(file = '63_00_DML_Print_All_ML_Parameters.R')
...
cat('* MAIN PROGRAM : Model Selection and DML procedure ... ', '\n')
{
  ...
  for(i in 1:times.split.dml){
    ...
    cat('* FILE : 63_00_Model_Selection.R, Iteration = ', i , '\n')
    source(file = '63_00_Model_Selection.R')
    ...
    cat('* Actual DML, Iteration = ', i , '\n')
    {
      ...
      cat('* FILE : 63_00_DML_Procedure_for_GLM.R, Iteration = ', i , '\n')
      source(file = '63_00_DML_Procedure_for_GLM.R')
