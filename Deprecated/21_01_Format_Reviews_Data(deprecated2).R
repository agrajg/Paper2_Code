...
cat('----------------------', '\n')
{
  ...
  packages<-c("foreach", "doParallel", "tidyr", "dplyr", "caret", "stringr", "gmm", "magrittr", "tidytext", "tm", "SnowballC")
...

{
  ...
  stopwords.list <- stopwords("english")
}
...
cat('----------------------', '\n')
{
  review.data <- readRDS(file = paste(project.path,"Output/TEMP/","01_01_Reviews_data_plain.rds", sep=""))
  review.data <- review.data %>% mutate(rev_date = date) 
  review.data$rev_date <- as.Date(review.data$rev_date, "%d%b%Y")
  review.data <- review.data %>% arrange(propertyid, rev_date) 
}

cat('Create a corpus ...', '\n')
review_corpus = Corpus(VectorSource(review.data$comments))
review_corpus
...
review_corpus <- tm_map(review_corpus, addspace, "[[:punct:]]")
review_corpus <- tm_map(review_corpus, content_transformer(tolower))
# review_corpus <- tm_map(review_corpus, removeNumbers)
review_corpus <- tm_map(review_corpus, removePunctuation)
review_corpus <- tm_map(review_corpus, removeWords, stopwords.list)
...
writeLines(head(strwrap(review_corpus[[2]]), 15))
...
review_dtm
inspect(review_dtm[1:5, 10:17]) 

review_dtm_tfidf <- DocumentTermMatrix(review_corpus, control = list(weighting = weightTfIdf))
review_dtm_tfidf
inspect(review_dtm_tfidf[1:5, 10:17]) 





review_dtm_dense = removeSparseTerms(review_dtm, 0.99)
review_dtm_dense
review_dtm_tfidf_dense = removeSparseTerms(review_dtm_tfidf, 0.999)
review_dtm_tfidf_dense

review.data = cbind(review.data, as.matrix(review_dtm_tfidf_dense))


# Collect reviews that are posted on same day
text.df <- review.data %>%
  select(-reviewer_id, -id,-reviewer_name,-comments) %>% 
