...
for(i in 1:times.split.dml) { 
  ...
  for(j in 1:crossfit.K){
    ...
    for(y in Y){
      ...
      print(y)
      model.parameters <- best_glm_grid_no_lambda_search_df %>% filter(outcome == y)
      fam = "gaussian"
    ...
    for(d in D){
      cat('target variable is : ' , '\n')
      print(d)
      model.parameters <- best_glm_grid_no_lambda_search_df %>% filter(outcome == d)
      fam = "gaussian"
      if(model.parameters$outcome_type == "bin"){ fam = "binomial" }
      resdx <- vector()
    ...
    # for(z in Z[1:4]){       # *** Remove this in the final program ***
      cat('target variable is : ' , '\n')
      print(z)
      model.parameters <- best_glm_grid_no_lambda_search_df %>% filter(outcome == z)
      fam = "gaussian"
      if(model.parameters$outcome_type == "bin"){ fam = "binomial" }
      reszx <- vector()
